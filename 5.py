...
#4.	F(1) = 1; G(1) = 1; F(n) = 2*F(n–1) – G(n–1), G(n) = 2*F(n–1) + G(n–1), при n >=2
import time
...
def iter_f(n):
    ...
    for m in range(3, n + 1):
        fn[m] = 2 * fn[m - 1] - gn[m - 1]
        gn[m] = 2*fn[m - 1] + gn[m - 1]
    return fn[n]

try:
    n = int(input('Введите натуральное число n >= 2: '))
    while n < 2:  # Ошибка в случае введения не натурального числа
        n = int(input('Вы ввели число меньше 2. В условии указано, что число должно быть больше или равно 2. '))
